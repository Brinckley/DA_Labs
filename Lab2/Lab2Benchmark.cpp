...
int main()
{
    ...
    steady_clock::duration totalMapTimer = steady_clock::now() - steady_clock::now();
    steady_clock::duration totalTreeTimer = totalMapTimer;
    steady_clock::time_point start;

	std::map<std::string, TUll> mapSTD;
    ...
    while (scanf("%s", switcher) != EOF) {
        ...
        if (switcher[0] == '+') { //add
            ...
            tree.Insert(tmp);
            totalTreeTimer += steady_clock::now() - start;

            start = steady_clock::now();
            if (mapSTD.find(tmp.mword) == mapSTD.end()) {
				mapSTD[tmp.mword] = tmp.mkey;
			}
            totalMapTimer += steady_clock::now() - start;
        }
        else {
    ...
    std::cerr << "Tree work time: " << std::chrono::duration_cast<std::chrono::milliseconds>(totalTreeTimer).count() << " ms\n";
    std::cerr << "Map work time: " << std::chrono::duration_cast<std::chrono::milliseconds>(totalMapTimer).count() << " ms\n";
